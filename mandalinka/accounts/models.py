...
class Address(models.Model):
    name = models.CharField(max_length=32, default="Domov",
                            verbose_name="Názov adresy")
    address = models.CharField(
        max_length=128, verbose_name="Adresa a číslo domu", help_text="Nezabudnite pridať číslo domu")
    note = models.TextField(max_length=256, blank=True,
                            verbose_name="Poznámka pre kuriéra", help_text="(zvonček, poschodie, ...)")
    city = models.CharField(max_length=100, verbose_name="Mesto")
    district = models.CharField(
        max_length=50, blank=True, verbose_name="Okres")
    postal = models.CharField(max_length=6, verbose_name="PSČ")
    country = models.CharField(
        max_length=32, blank=True, verbose_name="Krajina")
    coordinates = models.CharField(max_length=64)

    primary = models.BooleanField(default=True)

    user = models.ForeignKey(
        'accounts.User', related_name='addresses', on_delete=models.CASCADE)
...
def pick_primary_address(sender, instance, using, **kwargs):
    ...
    if not user:
        return
    prev_primary_address = user.addresses.exclude(
        pk=instance.pk).filter(primary=True)
    if prev_primary_address.count() == 0:
        return
...
class User(AbstractUser):
    ...
    phone = models.CharField(
        'Telefónne číslo', help_text='Môže byť použité počas doručovania', max_length=20, default='+421')
    newsletter = models.BooleanField(default=True)
    terms_conditions = models.BooleanField(blank=False)

    USERNAME_FIELD = 'email'
    REQUIRED_FIELDS = ('username', 'first_name', 'last_name',
                       'phone', 'terms_conditions')
    ...
    _is_email_valid = models.BooleanField(default=False,
                                          help_text="Designates whether this user has confirmed email. "
                                          )
    _is_payment_valid = models.BooleanField(default=False,
                                            help_text="Designates whether this user has a valid payment option. "
                                            )
    _is_subscribed = models.BooleanField(default=False,
                                         help_text="Designates whether this user is subscribed, has automatic payments turned on. "
                                         )
    ...
    food_preferences = models.ManyToManyField('management.Attribute', related_name="users",
                                              blank=True,
    ...
    alergies = models.ManyToManyField('management.Alergen', related_name="users",
    ...
    diet = models.ManyToManyField('management.Diet', related_name='users',
                                  blank=True, default=None,
    ...
    kitchen_accesories = models.ManyToManyField('management.KitchenAccesory', related_name='users',
                                                blank=True, default=None,
    ...
    class Meta(AbstractUser.Meta):
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(_is_subscribed=True, _is_payment_valid=True) | Q(
                    _is_subscribed=False),
                name='Payment is required for subscription'),
            models.CheckConstraint(
                check=Q(_is_payment_valid=True, _is_email_valid=True) | Q(
                    _is_payment_valid=False),
                name='Email confirmation is required for adding payment method'),
            models.CheckConstraint(
                check=Q(_is_subscribed=True, is_active=True) | Q(
                    _is_subscribed=False),
    ...
    def __delete_all_future_orders(self):
        self.order.filter(
            delivery_day__date__gte=datetime.date.today(), payed=False).delete()

    def populate_next_order(self):
        """This relies on the fact that there is at least one future order"""
        self.orders.filter(delivery_day__date__gte=datetime.date.today()).order_by(
            'date').first().automaticaly_generate()
